...
def mainGame():
    ...
    pipeVelX = -4  # Default speed
    if score > 5:
        pipeVelX = -5  # Faster pipes
    if score > 10:
        pipeVelX = -6  # Even faster
    ...
    while True:
        ...
        highScoreText = pygame.font.Font(None, 24).render(f"High Score: {high_score}", True, (255, 255, 255))
        SCREEN.blit(highScoreText, (SCREENWIDTH - 150, 10))  # Adjust position if needed
        pygame.display.update()
        FPSCLOCK.tick(FPS)
...
def gameOverScreen(score):
    ...
    SCREEN.blit(game_over_text, (SCREENWIDTH//10, SCREENHEIGHT//2))
    pygame.display.update()
